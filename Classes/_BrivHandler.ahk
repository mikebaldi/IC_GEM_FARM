class _BrivHandler extends _HeroHandler
{
    ...
    StackRestart(settings, obj)
    {
        ...
        while (this.Stacks < settings.TargetStacks AND i < 10)
        {
            ...
            currentZone := _MemoryHandler.GameInstance.ActiveCampaignData.currentAreaID.GetValue()
    ...
    StackFarmSetup(settings, funcObj)
    {
        g_Log.CreateEvent(A_ThisFunc)
        _VirtualKeyInputs.Priority("w")
        funcObj.WaitForTransition("w")
        funcObj.ToggleAutoProgress(0)
        funcObj.FallBackFromBossZone("w")
        startTime := A_TickCount
        elapsedTime := 0
        while(funcObj.IsCurrentFormation(settings.StackFormation) AND elapsedTime < 5000)
        {
            _VirtualKeyInputs.Priority("w")
            sleep, 100
            elapsedTime := A_TickCount - startTime
    ...
    BenchConditions(settings)
    {
        ...
        if (this.ActionList.Size() == 1 AND this.areaTransitioner.transitionDirection.GetValue() != 2 AND this.formation.transitionDir.GetValue() == 3 )
            return true
