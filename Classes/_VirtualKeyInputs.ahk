/*  A class to send key inputs to a specified window, similar to ControlSend.
    ...
    _VirtualKeyInputs.Generic( inputs* ) ;Generic method provides no confirmation if message was accepted by application.
    inputs* == Variadic parameter of inputs

    _VirtualKeyInputs.Priority( inputs* ) ;Priority method will attempt to resend inputs if the application does not respond with success.
    inputs* == Variadic parameter of inputs

    Note: Each input should be its own parameter, for example _VirtualKeyInputs.Priority( "a", "b" ) and not _VirtualKeyInputs.Priority( "ab" )
    Also accepts arrays of strings, use * when passing an array, for example _VirtualKeyInputs.Priority( AnArray* ) or _VirtualKeyInputs.Priority( ["a", "b"]* )
*/

class _VirtualKeyInputs
{
    inited := false
    ...
    GenericHWND(HWND, inputs*)
    {
        ControlFocus,, ahk_id %HWND%
        for k, v in inputs
        {
            PostMessage, 0x100, % this.KeyMap[ v ], 0,, ahk_id %HWND%
            PostMessage, 0x101, % this.KeyMap[ v ], 0xC0000001,, ahk_id %HWND%
        }
        Sleep, 10
    }

    PriorityHWND(HWND, inputs*)
    {
        keyState := [ "Down", "Up" ]
        ControlFocus,, ahk_id %HWND%
        for k, v in inputs
        {
            for l, b in keyState
            {
                SendMessage, % b == "Down" ? 0x100 : 0x101, % this.KeyMap[ v ], % b == "Down" ? 0 : 0xC0000001,, ahk_id %HWND%,,,, % this[ b ].Wait
                this[ b ].SetLastRun( i := 0 )
                while ( ErrorLevel AND this[ b ].GetLastRun() < this[ b ].MaxAttempts )
                {
                    SendMessage, % b == "Down" ? 0x100 : 0x101, % this.KeyMap[ v ], % b == "Down" ? 0 : 0xC0000001,, ahk_id %HWND%,,,, % this[ b ].Wait
    ...
    Init( window )
    {
        ...
        this.Down := new _VirtualKeyInputs.FailHandler( 10, 5 )
        this.Up := new _VirtualKeyInputs.FailHandler( 10, 5 )
